...
const {
  generateRandomToken,
} = require("../utils/security/generateRandomToken");
...
const login = async (email, password) => {
  ...
  const ispasswordCorrect = await secureOperations.comparePassword(
    password,
    user.password
  );
  ...
  const token = securityService.sign(
  ...
  return { token, refreshToken, role: user.role, email: user.email };
};
...
const refreshAccessToken = async (refreshToken) => {
  ...
  const newAccessToken = securityService.sign(
    { id: user.id, role: user.role },
    { expiresIn: "15m" }
  );
